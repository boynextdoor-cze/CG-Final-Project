...
struct Ray {
	...
	explicit Ray(Vec3f o, const Vec3f &dir, float t_min = RAY_DEFAULT_MIN,
	             float t_max = RAY_DEFAULT_MAX)
	    : origin(std::move(o)), direction(dir.normalized()), t_min(t_min),
	      t_max(t_max) {
		direction.normalize();
		inv_direction = {
		        std::fabs(direction[0]) < (float) 1e-10 ? (float) 1e10
		                                                : 1.f / direction[0],
		        std::fabs(direction[1]) < (float) 1e-10 ? (float) 1e10
		                                                : 1.f / direction[1],
		        std::fabs(direction[2]) < (float) 1e-10 ? (float) 1e10
		                                                : 1.f / direction[2]};
