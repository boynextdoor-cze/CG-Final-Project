...
bool AABB::intersect(const Ray &ray, float &t_in, float &t_out) {
	...
	float dir_frac_x = (ray.direction[0] == 0.0) ? 1.0e32f : 1.f / ray.direction[0];
	float dir_frac_y = (ray.direction[1] == 0.0) ? 1.0e32f : 1.f / ray.direction[1];
	float dir_frac_z = (ray.direction[2] == 0.0) ? 1.0e32f : 1.f / ray.direction[2];
	...
	t_in = std::max(std::max(std::min(tx1, tx2), std::min(ty1, ty2)),
	                 std::min(tz1, tz2));
	t_out = std::min(std::min(std::max(tx1, tx2), std::max(ty1, ty2)),
	                  std::max(tz1, tz2));
