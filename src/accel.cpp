...
BVHAccel::BVHAccel(std::vector<ObjectPtr> &p, int maxPrimsInNode,
	...
	root = recursiveBuild(object_info, 0, (int) objects.size(), total_nodes,
	                      ordered_objects);
	...
	//  nodes =
	//      (LinearBVHNode *)aligned_alloc(64, total_nodes *
...
BVHNodePtr BVHAccel::recursiveBuild(std::vector<BVHObjectInfo> &object_info,
	...
	if (objects_num == 1) {
		int first = (int) ordered_obj.size();
		for (int i = begin; i < end; i++) {
		...
		if (std::fabs(centroidBounds.pMax[dim] - centroidBounds.pMin[dim]) <
		    EPS * EPS) {
			int first = (int) ordered_obj.size();
			for (int i = begin; i < end; i++) {
				int index = object_info[i].index;
			...
			if (objects_num <= 2) {
				...
				for (int i = begin; i < end; i++) {
					int b =
					        (int) (B * centroidBounds.Offset(object_info[i].centroid)[dim]);
					// on the border
					if (b == B)
						b = B - 1;
					if (b < 0) {
				...
				for (int i = 0; i < B - 1; i++) {
					...
					cost[i] =
					        1.f + (float) pre_count * pre.SurfaceArea() / bounds.SurfaceArea();
				}
				for (int i = B - 1; i >= 1; i--) {
					suf = Union(suf, buckets[i].bounds);
					suf_count += buckets[i].count;
					cost[i - 1] +=
					        (float) suf_count * suf.SurfaceArea() / bounds.SurfaceArea();
				}
				...
				auto leaf_cost = (float) objects_num;
				if (objects_num > maxPrimsInNode || min_cost < leaf_cost) {
					// should not make this a leaf node
					BVHObjectInfo *pmid = std::partition(
					        &object_info[begin], &object_info[end - 1] + 1,
					        [=](const BVHObjectInfo &pi) {
						        int b = (int) (B * centroidBounds.Offset(pi.centroid)[dim]);
						        if (b == B)
							        b = B - 1;
						        return b <= min_split_index;
					        });
					mid = (int) (pmid - &object_info[0]);
...
void BVHAccel::recursiveCheck(const BVHNodePtr &node) {
	if (node->objects_num > 0) {
...
int BVHAccel::flattenBVHTree(const BVHNodePtr &node, int &offset) {
...
void BVHAccel::getIntersection(const BVHNodePtr &node, const Ray &ray,
