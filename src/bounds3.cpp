...
bool Bounds3::Overlaps(const Bounds3 &b1, const Bounds3 &b2) const {
...
bool Bounds3::IntersectP(const Ray &ray) const {
  ...
  if (t_l > t_r + EPS) return false;
  if (t_r < -EPS) return false;
  return true;
}
...
std::pair<bool, float> Bounds3::IntersectWithTime(const Ray &ray) const {
  ...
  t_l = l.cwiseMin(r).maxCoeff();
  t_r = l.cwiseMax(r).minCoeff();
  if (t_l > t_r + EPS) return {false, float_max};
  if (t_r < -EPS) return {false, float_max};
  return {true, t_l};
}
