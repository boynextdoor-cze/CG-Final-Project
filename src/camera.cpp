...
Ray Camera::generateRay(float dx, float dy) {
	...
	return Ray(position, (dx * right + dy * up + forward).normalized());
}

std::vector<Vec2f> Camera::getRaySamples(float dx, float dy, int num_samples) {
	std::vector<Vec2f> samples;
	int num_per_axis = (int) std::sqrt(num_samples);
	for (int i = 0; i < num_per_axis; ++i)
		for (int j = 0; j < num_per_axis; ++j) {
			float x = ((float) i + 0.5f) / (float) num_per_axis;
			float y = ((float) j + 0.5f) / (float) num_per_axis;
			samples.emplace_back(dx + x, dy + y);
		}
	return samples;
}
