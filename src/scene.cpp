...
void initSceneFromConfig(const Config &config, std::shared_ptr<Scene> &scene) {
	...
	// Generate mesh on NURBS
  {
    ...
    YELLOW = std::make_shared<IdealDiffusion>(Vec3f(0.7, 0.7, 0.04));
		...
		std::shared_ptr<NURBS> tmpDuck1 = std::make_shared<NURBS>(13, 12, 4, 4, 
								readControlPoints(R"(../assets/duck1.ctrlpts)", 13, 12), 
								readWeights(R"(../assets/duck1.weights)", 13, 12), 
								std::vector<float>({-1.5708, -1.5708, -1.5708, -1.5708, -1.0472, -0.523599, 0, 0.523599, 0.808217,
                       1.04015, 1.0472, 1.24824, 1.29714, 1.46148, 1.5708, 1.5708, 1.5708, 1.5708}),
								std::vector<float>({-3.14159, -3.14159, -3.14159, -3.14159, -2.61799, -2.0944, -1.0472, -0.523599,
                       6.66134e-016, 0.523599, 1.0472, 2.0944, 2.61799, 3.14159, 3.14159, 3.14159, 3.14159}),
								GREEN);
		tmpDuck1->init();

		std::shared_ptr<NURBS> tmpDuck2 = std::make_shared<NURBS>(8, 9, 4, 4, 
								readControlPoints(R"(../assets/duck2.ctrlpts)", 8, 9), 
								std::vector<float>({0, 0, 0, 0, 0.145456, 0.265731, 0.436096, 0.583258, 0.847704, 1, 1, 1, 1}),
								std::vector<float>({0, 0, 0, 0, 0.179541, 0.317924, 0.485586, 0.507528, 0.709398, 0.813231, 1, 1, 1, 1}),
								YELLOW);
		tmpDuck2->init();

		std::shared_ptr<NURBS> tmpDuck3 = std::make_shared<NURBS>(5, 5, 4, 4, 
								readControlPoints(R"(../assets/duck3.ctrlpts)", 5, 5), 
								std::vector<float>({0, 0, 0, 0, 0.333333, 0.666667, 1, 1, 1, 1}),
								std::vector<float>({0, 0, 0, 0, 0.333333, 0.666667, 1, 1, 1, 1}),
								YELLOW);
		tmpDuck3->init();
