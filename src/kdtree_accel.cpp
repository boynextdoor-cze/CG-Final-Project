...
KDTreeAccel::KDTreeAccel(std::vector<ObjectPtr> &_objects, int _intersectCost,
    ...
    : intersectCost(_intersectCost), traversalCost(_traversalCost),
      maxObjects(_maxObjects), emptyBonus(_emptyBonus), objects(_objects) {
	if (maxDepth <= 0)
		maxDepth = (int) std::round(8 + 1.3f * std::log2(objects.size()));
	...
	recursiveBuild(bound, objectBounds, objectIndex.data(), (int) objects.size(),
	...
	printf("\rKDTree Generation complete: \nTime Taken: %i hrs, %i mins, %i "
	       "secs\n\n",
	       hrs, mins, secs);
}
...
void KDTreeAccel::recursiveBuild(const Bounds3 &curBound,
	...
	float bestCost = 1e9;
	float oldCost = (float) intersectCost * (float) curTotalObjects;
	...
	for (int axis = 0; axis < 3; axis++) {
		...
		for (int i = 0; i < 2 * curTotalObjects; i++) {
			...
			if (curEdgeT > curBound.pMin[axis] && curEdgeT < curBound.pMax[axis]) {
				...
				float cost =
				        (float) traversalCost +
				        (float) intersectCost * (1 - emptyBonus) *
				                (pBelow * (float) totalBelow + pAbove * (float) totalAbove);
...
bool KDTreeAccel::getIntersection(const Ray &ray,
	...
	if (!bound.IntersectP(ray, tMin, tMax))
		return false;
	...
	while (node != nullptr) {
		...
		if (!node->IsLeaf()) {
			...
			if (curTotalObjects == 1) {
				...
				if (ptr->intersect(ray, interaction))
					hit = true;
			} else {
				for (int i = 0; i < curTotalObjects; ++i) {
