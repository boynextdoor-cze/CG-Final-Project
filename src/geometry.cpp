...
bool isSingularTriangle(const Vec3f &v0, const Vec3f &v1, const Vec3f &v2) {
...
Triangle::Triangle(const Vec3f &_v0, const Vec3f &_v1, const Vec3f &_v2,
                   Vec3f _n0, Vec3f _n1, Vec3f _n2, std::shared_ptr<BSDF> _bsdf)
    : v0(_v0), v1(_v1), v2(_v2), n0(std::move(_n0)), n1(std::move(_n1)),
      n2(std::move(_n2)), bsdf(std::move(_bsdf)), bound(_v0, _v1, _v2) {}
...
void TriangleMesh::buildBVH() {
	...
	for (int i = 0; i < v_indices.size() / 3; i++) {
		Vec3i v_idx(v_indices[3 * i], v_indices[3 * i + 1], v_indices[3 * i + 2]);
		Vec3i n_idx(n_indices[3 * i], n_indices[3 * i + 1], n_indices[3 * i + 2]);
		if (!isSingularTriangle(vertices[v_idx[0]], vertices[v_idx[1]],
		                        vertices[v_idx[2]]))
			objects[cnt++] =
