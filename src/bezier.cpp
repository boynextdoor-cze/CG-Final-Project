...
NURBS::NURBS(int _m, int _n, int _k, int _l,
             ...
             std::shared_ptr<BSDF> &_bsdf)
    : u_m(_m + _k), u_n(_m), u_p(_k - 1), v_m(_n + _l), v_n(_n), v_p(_l - 1),
      k(_k), l(_l), controlPoints(_controlPoints), knotM(_knotM), knotN(_knotN),
      bsdf(_bsdf) {
...
void NURBS::setControlPoint(int i, int j, Vec3f point) {
	controlPoints[i][j] = std::move(point);
}
...
std::pair<float, float> NURBS::evaluateN(std::vector<float> &knot, float t,
	...
	for (auto iteration = 2; iteration < k; iteration++) {
		for (auto index = 0; index + iteration <= m; index++) {
			tmp[index] = multiplyForBSplineBasisFunction(tmp[index], t - knot[index],
			                                             knot[index + iteration - 1] -
			                                                     knot[index]) +
...
Vertex NURBS::evaluateWithNormal(float u, float v) {
	...
	int p = u_p, q = v_p;
	int c = static_cast<int>(upper_bound(knotM.begin(), knotM.end(), u) -
	                         knotM.begin() - 1);
	int d = static_cast<int>(upper_bound(knotN.begin(), knotN.end(), v) -
	                         knotN.begin() - 1);
	int s = 0, t = 0;
	...
	Vec3f derivative_u = (float) p * u_control_point_next.w() /
	                     ((knotM[c + 1] - u) * u_control_point.w()) *
	                     (u_point_next - point);
	Vec3f derivative_v = (float) q * v_control_point_next.w() /
...
#endif
	// #undef MY_DEBUG
	return Vertex{point, normal, derivative_u, derivative_v};
}
...
#endif
	// #undef MY_DEBUG
	return Vertex{to3f(point), to3f(normal), to3f(derivative_u),
	              to3f(derivative_v)};
}
	...
	// calculate additional refined knots in each row
	{
		...
		for (int i = u_p; i < u_m - u_p; i++) {
			...
			for (int j = 1; j <= max_n; j++)
				knotU_insert.push_back(knotM[i] + (float) j * segment);
		}
	}
#ifdef MY_DEBUG
		...
		for (int i = v_p; i < v_m - v_p; i++) {
			...
			for (int ctrl_i = 0; ctrl_i <= u_n; ctrl_i++) {
				...
				for (int ctrl_j = i - v_p + 1; ctrl_j <= i; ctrl_j++)
					V[ctrl_j] = (float) v_p / (knotN[ctrl_j + v_p] - knotN[ctrl_j]) *
					            (controlPoints[ctrl_i][ctrl_j] -
					             controlPoints[ctrl_i][ctrl_j - 1]);
				for (int ctrl_j = i - v_p + 2; ctrl_j <= i; ctrl_j++)
					A[ctrl_j] = (float) (v_p - 1) /
					            (knotN[ctrl_j + v_p - 1] - knotN[ctrl_j]) *
					            (V[ctrl_j] - V[ctrl_j - 1]);
				for (int j = i - v_p + 1; j <= i; j++)
					sum_V += V[j].norm();
				for (int j = i - v_p + 2; j <= i; j++)
					max_A = std::max(max_A, A[j].norm());
				sum_V = std::sqrt(sum_V / (float) v_p);
				max_n = std::max(
				        max_n,
			...
			for (int j = 1; j <= max_n; j++)
				knotV_insert.push_back(knotN[i] + (float) j * segment);
		}
	}
#ifdef MY_DEBUG
...
NURBS::generateMesh(SamplingMode mode, int sampleMSize, int sampleNSize) {
...
void insertKnot(std::vector<float> &knot, std::vector<Vec4f> &homoControlPoints,
	...
	int k = static_cast<int>(upper_bound(knot.begin(), knot.end(), u) -
	                         knot.begin() - 1);
	int s = 0;
...
IntervalObject::IntervalObject(std::shared_ptr<NURBS> &_surface, int _i, int _j,
                               std::shared_ptr<BSDF> _bsdf)
    : surface(_surface), i(_i), j(_j), bsdf(std::move(_bsdf)) {
	updateBounds();
}
...
bool IntervalObject::intersect(const Ray &ray, Interaction &interaction) const {
	...
	for (int iter = 0; iter < MAX_ITER; iter++) {
		if (curIteration.norm < EPS) {
			...
			interaction.dist = (float) t;
			interaction.pos = ray((float) t);
		...
		if (std::fabs(det) < EQ_EPS) {
			...
			if (nextIteration.norm - curIteration.norm > 0)
				return false;
